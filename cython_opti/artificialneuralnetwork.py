...
import gradient_cython
import predict_cython
import cost_function_cython
...
def callbackF(input_layer_size, hidden_layer_size, num_labels, X, y, lmbda, test, test_label, theta_k):
	...
	# use the Cython reshape function
	Theta1, Theta2 = gradient_cython.reshape(theta_k, input_layer_size, hidden_layer_size, num_labels)

	# use the Cython cost function
	J = cost_function_cython.cost_function(theta_k, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda)
	# use the Cython predict function
	y_pred = predict_cython.predict(Theta1, Theta2, X)
	accuracy = np.sum(1.*(y_pred==y))/len(y)
	Js_train = np.append(Js_train, J)

	J_test = cost_function_cython.cost_function(theta_k, input_layer_size, hidden_layer_size, num_labels, test, test_label, lmbda)
	# use the Cython predict function
	test_pred = predict_cython.predict(Theta1, Theta2, test)
...
def main():
	...
	theta0 = np.concatenate((Theta1.flatten(), Theta2.flatten())).astype(np.float64)
	J = cost_function_cython.cost_function(theta0, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda)
	print('initial cost function J =', J)
	train_pred = predict_cython.predict(Theta1, Theta2, train)
	print('initial accuracy on training set =', np.sum(1.*(train_pred==train_label))/len(train_label))
	global Js_train
	global Js_test
	Js_train = np.array([J])
	J_test = cost_function_cython.cost_function(theta0, input_layer_size, hidden_layer_size, num_labels, test, test_label, lmbda)
	...
	# use the Cython gradient function and the Cython cost function
	theta = optimize.fmin_cg(cost_function_cython.cost_function, theta0, fprime=gradient_cython.gradient, args=args, callback=cbf, maxiter=MAX_ITER)

	# use the Cython gradient function
	Theta1, Theta2 = gradient_cython.reshape(theta_best, input_layer_size, hidden_layer_size, num_labels)
